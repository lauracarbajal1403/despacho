...
export default function HomePage() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* ── LOGOS ── */}
      <section style={{ padding: isMobile ? '32px 20px' : '48px 24px', backgroundColor: '#1a2e4a', borderBottom: '1px solid #d0eeea' }}>
        <div style={{ maxWidth: '1100px', margin: '0 auto', textAlign: 'center' }}>
          <p style={{ fontSize: '11px', fontWeight: 700, color: '#4db8a8', letterSpacing: '0.12em', marginBottom: '24px' }}>
            CONFÍAN EN NOMMY PARA SUS DESPACHOS
          </p>
          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: isMobile ? '24px' : '40px', flexWrap: 'wrap' }}>
            {LOGOS.map((logo) => (
              <img key={logo.alt} src={logo.src} alt={logo.alt}
                style={{ height: isMobile ? '28px' : '36px', width: 'auto', objectFit: 'contain', opacity: 0.55, filter: 'grayscale(1)', transition: 'opacity 0.2s' }}
