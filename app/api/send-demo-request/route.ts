...
export async function POST(request: Request) {
  try {
    ...
    const { nombre, email, telefono, empresa, colaboradores, recipientEmail } = body
        ...
        <body style="font-family: Arial, sans-serif; background:#f5f5f5; padding:20px;">
          <div style="max-width:600px;margin:0 auto;background:white;border-radius:8px;overflow:hidden;">
            ...
            <div style="padding:30px;">
              ...
              <p><strong>Despacho:</strong> ${empresa || "No proporcionado"}</p>
              <p><strong>Número de colaboradores:</strong> ${colaboradores || "No proporcionado"}</p>
            </div>
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: recipient,
      subject: `Nueva solicitud de demo de NOMMY - ${empresa || nombre}`,
      html: htmlContent,
    })
